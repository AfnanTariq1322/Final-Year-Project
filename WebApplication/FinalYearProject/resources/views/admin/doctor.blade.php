...
<style>
    ...
    ***********************************-->
    ...
    @endif
        ...
        <tr>
             <td>
                <img src="{{ $user->image_url }}" alt="Profile Picture" width="40" height="40">
            </td>
            <td>{{ $user->name }}</td>
            <td>{{ $user->email }}</td>
            <td class="badge badge-sm badge-secondary">{{ $user->specialty }}</td>

            <td>{{ $user->phone }}</td>
            <td>
                <span class="badge {{ $user->status === 'verified' ? 'badge-success' : 'badge-danger' }}">
                    {{ $user->status }}
                </span>
            </td>
            <td>
                <div class="d-flex">
                    ...
                    <form action="{{ route('toggle.verification', $user->doctor_id) }}" method="POST" class="mr-2">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="btn btn-warning btn-xs sharp">
                            {{ $user->status === 'verified' ? 'Unverified' : 'Verify' }}
                        </button>
                    </form>

                     <!-- Delete Button -->
                     <form action="{{ route('delete.doctor', $user->doctor_id) }}" method="POST">
                        @csrf
                        @method('DELETE')
